			...
			<li class="product <?php /*tax*/ $posttags = get_the_terms($post->ID, 'lineas',''); if ($posttags) { foreach($posttags as $tag) { echo $tag->slug . ' ';} } ?>">
				...
				<div class="row">
					...
					<div class="text columns medium-3 medium-offset-1">
						...
						<?php the_excerpt(); ?>
						<a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to%s', 'soccerpro' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark" class="button radius my-buttom">Saber más</a>
					</div>
